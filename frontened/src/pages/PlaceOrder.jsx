...
const {navigate,backendUrl,token,cartItems,setCartItems,getCartAmount,delivery_fee,products} = useContext(ShopContext); 
 ...
 const onSubmitHandler = async (event)=>{
  ...
  const lineItems = orderItems.map(item => ({
    price_data: {
      currency: 'usd', // or 'inr' if you're using INR
      product_data: {
        name: item.name,
      },
      unit_amount: item.price * 100, // Stripe expects price in cents or paise
    },
  ...
  const responseStripe = await axios.post(
    ...
    {
      line_items: lineItems, // ✅ required by Stripe
      items: orderItems,     // Optional: for your own backend record
      address: formData,
      amount: getCartAmount() + delivery_fee,
    },
    { headers: { token } }
